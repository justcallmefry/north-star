...
exports.resolveContentDayIndex = resolveContentDayIndex;
exports.pickNextContentDayIndex = pickNextContentDayIndex;
/**
 ...
 */
function resolveContentDayIndex(sessionDate, stored, maxDay, fallbackFromDate) {
    const clamp = (n) => {
        if (!Number.isFinite(n) || maxDay < 1)
            return 1;
 ...
 */
function pickNextContentDayIndex(params) {
    const { maxDay, sessions, fallbackFromDate } = params;
    if (maxDay < 1)
        return 1;
    const lastUse = new Map();
    for (const s of sessions) {
        const idx = resolveContentDayIndex(s.sessionDate, s.contentDayIndex, maxDay, fallbackFromDate);
